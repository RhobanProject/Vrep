...
//				if (!_mergeImported)
	//				printf("Too many meshes to import. Building one single merged shape... ");
				...
				std::string objBaseName("ColladaImportedShape_merged");
				std::string objName(objBaseName);
				int suffix=-1;
				while (simSetObjectName(anObj,objName.c_str())==-1)
					objName=objBaseName+boost::lexical_cast<std::string>(suffix++);
//				printf("done!\n");
			}
			loadSuccess=true;
		}
...
bool CColladaDialog::addNodeToScene(const COLLADAImporter* importer,const mat4& parentMatrix,int parentObjectID,const SceneNode* node,float jointAndDummySizes,bool onlyShapes,std::vector<float>* _allVert,std::vector<int>* _allInd,float importScale)
{
	...
	for (int i=0;i<int(node->getTransformations().size());i++)
	{
		...
		if (node->getTransformations()[i]->getTransformationType()==Transformation::Translate)
		{
			...
			if (!onlyShapes)
			{
				...
				int suffix=1;
				std::string objBaseName("colladaTransformationNode");
				std::string objName(objBaseName);
				while (simSetObjectName(jh,objName.c_str())==-1)
					objName=objBaseName+boost::lexical_cast<std::string>(suffix++);
				parentObjectID=jh;
			}
	...
	if (mesh!=NULL)
	{
		...
		if (_allInd==NULL)
		{
			...
			if (theObjectHandle!=-1)
			{
				...
				int suffix=1;
				std::string objBaseName(node->getName());
				std::string objName(objBaseName);
				while (simSetObjectName(theObjectHandle,objName.c_str())==-1)
					objName=objBaseName+boost::lexical_cast<std::string>(suffix++);
			}
		}
	}
	...
	{ // we use a dummy instead
		if (!onlyShapes)
		{
			...
			simSetObjectParent(theObjectHandle,parentObjectID,true);
			simSetObjectName(theObjectHandle,node->getName().c_str());
		}
	}
	for (int i=0;i<int(node->getChildren().size());i++)
